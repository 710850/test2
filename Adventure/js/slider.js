$(function() {
 var slider = $('.slider__content'),
     ...
     sliderInterval = 1000000000,                              // Интервал смены слайдов
     animateTime = 900,                                 // Время смены слайдов
     course = 1,                                         // Направление движения слайдера (1 или -1)
     margin = - slideWidth;                              // Первоначальное смещение слайдов

 last_slide.clone().prependTo('.slider__content');   // Копия последнего слайда помещается в начало.
slide.eq(1).clone().appendTo('.slider__content');    // Копия первого слайда помещается в конец.
slider.css('margin-left', -slideWidth);         // Контейнер .slider__content сдвигается влево на ширину одного слайда.
 ...
 function animate(){
  if (margin==-slideCount*slideWidth-slideWidth){     // Если слайдер дошел до конца
   slider.css({'marginLeft':-slideWidth});           // то блок .slider__content возвращается в начальное положение
   margin=-slideWidth*2;
  }else if(margin==0 && course==-1){                  // Если слайдер находится в начале и нажата кнопка "назад"
   slider.css({'marginLeft':-slideWidth*slideCount});// то блок .slider__content перемещается в конечное положение
   margin=-slideWidth*slideCount+slideWidth;
  }else{                                              // Если условия выше не сработали,
   margin = margin - slideWidth*(course);              // значение margin устанавливается для показа следующего слайда
  }
  slider.animate({'marginLeft':margin},animateTime);  // Блок .slider__content смещается влево на 1 слайд.
 }
